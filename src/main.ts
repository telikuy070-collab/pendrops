...
function bindEvents(
  ...
  searchInput?.addEventListener('input', (e) => actions.setFilter('search', (e.target as HTMLInputElement).value));
  dayFilter?.addEventListener('change', (e) => actions.setFilter('day', (e.target as HTMLSelectElement).value));
  ...
  let pullStart = 0;
  scheduleContainer?.addEventListener('touchstart', (e: TouchEvent) => {
    if ((e.target as HTMLElement).closest('.card')) return;
    const touch = e.touches[0];
    if (touch) pullStart = touch.clientY;
  }, { passive: true });
  
  scheduleContainer?.addEventListener('touchmove', (e: TouchEvent) => {
...
function renderSubgroupPicker(prefsService: PrefsServiceType): void {
  ...
  const subgroups = Array.from(new Set(groupLessons.map(l => l.subgroup).filter(Boolean)))
    .sort((a, b) => {
