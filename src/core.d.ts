...
// cell.js
export function parseCell(text: string): { subject: string; type: string; teacher: string; room: string; isExam: boolean } | null;
export function parseGroupCode(text: string): { code: string; subgroup: string } | null;
export function splitSubs(text: string): string[];

// day.js
export function detectDay(text: string): 'Понедельник' | 'Вторник' | 'Среда' | 'Четверг' | 'Пятница' | 'Суббота' | 'Воскресенье' | null;

// timing.js
export function parseTimeRange(time: string): { start: string; end: string };
export function lessonState(lesson: { time?: string | null }, currentTime: string): 'upcoming' | 'now' | 'past' | 'idle';
export function getCurrentTime(): string;
export function getTodayName(): 'Понедельник' | 'Вторник' | 'Среда' | 'Четверг' | 'Пятница' | 'Суббота' | 'Воскресенье';
...
export function loadState(): Promise<{ sheets: Record<string, any[]>; current: string; group: string }>;
