...
export async function loadPreferencesUseCase(
  storage: IStorage
): Promise<UserPreferences> {
  return await storage.get<UserPreferences>('user_prefs') || {
    currentSheetId: '',
    currentGroup: '',
    activeSubgroup: '',
    hiddenSheets: []
...
export function getSheetsUseCase(lessons: Lesson[]): Sheet[] {
  ...
  for (const lesson of lessons) {
    ...
    if (!existing) {
      sheetMap.set(lesson.sheetId, { name: lesson.sheetId, lessonCount: 1, order: lesson.dayOrder });
    } else {
...
export function getGroupsUseCase(lessons: Lesson[], sheetId: string): Group[] {
  ...
  for (const lesson of sheetLessons) {
    ...
    if (!existing) {
      groupMap.set(lesson.group, { code: lesson.group, subgroups: new Set([lesson.subgroup].filter(Boolean)) });
    } else {
